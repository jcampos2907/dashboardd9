...
const D3Arrow: React.FC<ArrowProps> = ({ x1, y1, x2, y2, color = '#000', thickness = 1, animated = true, country, year }) => {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            setCoordinates({ y1, x1, x2: adjustedX2, y2: adjustedY2 });
        }, 300);

        console.log(coordinates)
    }, [x1, y1, x2, y2, selectedYear]);
    ...
    return <svg x1={x1} y1={y1} x2={coordinates.x2} y2={coordinates.y2} ref={svgRef} className={classNames} id={'arrow' + country.replace(/ /g, "_").toLowerCase()} />;
};
